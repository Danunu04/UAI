...
namespace INGENIERIA_DE_SOFTWARE_VINICOLA
{
    public partial class GestionUsuarios : Form, IObserver
    {
        ...
        private void btnAplicar_Click(object sender, EventArgs e)
        {
            if(txtPanel.Text == "Modo añadir")
            {
                if(ValidarCampos() == true)
                {
                    ...
                    actualizarGrilla();
                    evento.RegistrarEvento(new Evento(SessionManager.ObtenerInstancia().Usuario.UserName, DateTime.Now,"Usuarios" ,"AgregarUsuario", 2));
            ...
            if(txtPanel.Text == "Modo modificar")
            {
                if(ValidarCampos() == true)
                {
                    ...
                    actualizarGrilla();
                    evento.RegistrarEvento(new Evento(SessionManager.ObtenerInstancia().Usuario.UserName, DateTime.Now, "Usuarios", "Usuari modificado", 2));
            ...
            if(txtPanel.Text == "Modo eliminar")
            {
                ...
                actualizarGrilla();
                evento.RegistrarEvento(new Evento(SessionManager.ObtenerInstancia().Usuario.UserName, DateTime.Now, "Usuarios", "Usuario eliminado", 2));
            ...
            if (txtPanel.Text == "Modo desbloquear")
            {
                ...
                else if(usuario.RevisarBloqueado(Convert.ToInt32(numericUpDown1.Value)) == false)
                {
                    ...
                    evento.RegistrarEvento(new Evento(SessionManager.ObtenerInstancia().Usuario.UserName, DateTime.Now, "Usuarios", "Usuario bloqueado", 2));
                }
                else
                {
                    ...
                    evento.RegistrarEvento(new Evento(SessionManager.ObtenerInstancia().Usuario.UserName, DateTime.Now, "Usuarios", "Usuario desbloqueado", 2));
                }
                actualizarGrilla();
            ColorearInactivos();
            }
        ...
        private void LlenarComboBox()
        {
            ...
            comboBox1.ValueMember = "CodFamilia";
        }
